...
async function modelDownloadAll(urls, opts) {
  const promises = [];
  const downloader = new Downloader();
  let multipleOptionFlag = false;
  if (urls.length == opts.length) {
    multipleOptionFlag = true;
  } else if (opts.length > 1) {
    opts = opts[0];
  }
  for (let i = 0; i < urls.length; i++) {
    const url = urls[i];
    const opt = multipleOptionFlag ? opts[i] : opts;
    promises.push(downloader.download(url, opt));
  }
...
exports.modelDownloadAll = modelDownloadAll;
